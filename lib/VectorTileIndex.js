...
function VectorTileIndex (geojson, db, options) {
  if (!(this instanceof VectorTileIndex)) return new VectorTileIndex(geojson, db, options)
  var self = this
  ...
  self.bbox = options.bbox || geojsonExtent(geojson)
  self.tileIndex = geojsonvt(geojson, {
    maxZoom: self.zMax,
      ...
      series(range(tileBounds.xMin, tileBounds.xMax + 1).map(function (x) {
        return function (done) {
          ...
          series(range(tileBounds.yMin, tileBounds.yMax + 1).map(function (y) {
            return function (done) {
              console.log(z, x, y)
              var tile = self.tileIndex.getTile(z, x, y)
              if (tile !== null) {
                var pbfOptions = {}
                pbfOptions['data'] = tile
